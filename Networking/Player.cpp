...
template <class T>
MultiPlayer<T>::MultiPlayer(const Names& Nms, int id) : Player(Nms), send_to_self_socket(0)
{
...
template <>
void MultiPlayer<int>::setup_sockets(
    const vector<string>& names, const vector<int>& ports, int id_base, ServerSocket& server)
{
    ...
    for (int i = player_no; i < nplayers; i++)
    {
        ...
        else
        {
            ...
            fprintf(stderr, "Setting up client to %s:%d with id 0x%x\n", names[i].c_str(), ports[i],
                pn);
#endif
            int ppc_socket_flags = get_socket_flag();
            usleep(ppc_socket_flags * 1000 * 1000);
...
void Player::partial_broadcast(const vector<bool>& senders, vector<octetStream>& os) const
{
    partial_broadcast(senders, vector<bool>(num_players(), senders[my_num()]), os);
}
